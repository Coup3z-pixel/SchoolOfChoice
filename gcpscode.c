...
partial_alloc NEW_simple_GCPS_alloc(input_sch_ch_prob* input, int allowed_speed_type) {
  run_data* data;
  ...
  answer = NEW_GCPS_allocation(input, allowed_speed_type, data); 
...
partial_alloc NEW_GCPS_allocation(input_sch_ch_prob* input, int allowed_speed_type,
				  run_data* data) {
  partial_alloc feasible_guide;
  pivot_list probe_list;
  process_scp input_pr_scp;
  ...
  return NEW_GCPS_allocation_with_guide(&input_pr_scp, allowed_speed_type,
					&feasible_guide, &probe_list, data);
}
...
partial_alloc NEW_simple_GCPS_alloc_with_guide(input_sch_ch_prob* input, int allowed_speed_type,
					   partial_alloc* feasible_guide) {
  pivot_list probe_list;
  ...
  answer = NEW_GCPS_allocation_with_guide(&input_pr_scp, allowed_speed_type,
					  feasible_guide, &probe_list, data);
...
partial_alloc NEW_GCPS_allocation_with_guide(process_scp* input,
  ...
  time_left = NEW_compute_until_next_critical_pair(input, feasible_guide, &final_alloc,
						   allowed_speed_type,
						   probe_list,
						   &P_subset, &J_subset,
						   data);
  ...
  NEW_descend_to_left_subproblem(input, &final_alloc, allowed_speed_type, feasible_guide,
				 probe_list, &P_subset, &J_subset, data);
  
  NEW_descend_to_right_subproblem(input, &final_alloc, allowed_speed_type, feasible_guide,
				  probe_list, &P_subset, &J_subset, data);
...
void NEW_descend_to_left_subproblem(process_scp* working_scp,
				    partial_alloc* final_alloc,
				    int allowed_speed_type,
				    partial_alloc* feasible_guide, 
				    pivot_list* probe_list,
  ...
  left_increment = NEW_GCPS_allocation_with_guide(&left_scp, allowed_speed_type,
						  &left_feas_guide, &left_list, data);
...
void NEW_descend_to_right_subproblem(process_scp* working_scp,
				     partial_alloc* final_alloc,
				     int allowed_speed_type,
				     partial_alloc* feasible_guide, 
				     pivot_list* probe_list,
  ...
  right_increment = NEW_GCPS_allocation_with_guide(&right_scp, allowed_speed_type,
						   &right_feas_guide, &right_list, data);
