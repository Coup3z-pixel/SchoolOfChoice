...
void make_example(int nsc, int no_students_per_school, int school_capacity,
  ...
  answer.no_students = nsc * no_students_per_school;
  answer.no_schools = nsc;
  answer.quotas = malloc(nsc * sizeof(int));
  for (j = 1; j <= nsc; j++) {
    answer.quotas[j-1] = school_capacity;
  }
  ...
  print_makex_output(nsc, no_students_per_school, school_capacity, school_valence_std_dev,
