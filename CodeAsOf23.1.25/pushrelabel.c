...
struct network_flow set_capacities(struct process_scp* input) {
  ...
  for (i = 1; i <= nst; i++) {
    for (j = 1; j <= nsc; j++) {
      set_entry(&(answer.flows), i, j, is_eligible(input, i, j) * input->time_remaining);      
    }
  }
...
double capacity(struct network_flow* cap, int arc_tail, int arc_head) {
  ...
  if (1 <= arc_tail && arc_tail <= nst && nst + 1 <= arc_head && arc_head <= nst + nsc) {
    ...
    /*
    if (arc_tail == 1 && arc_head == 8) {
      printf("With arc_tail = 1 and arc_head = 8 we are getting %1.3f.\n",
	     get_entry(&(cap->flows), arc_tail, arc_head - nst));
            exit(0); 
    }
    */
...
void relabel(int node, struct network_flow* cap, struct network_flow* pre, int* labels) {
  ...
  for (k = 1; k <= nst+nsc+1; k++) {
    ...
    /*
    fprintf(stderr, "When k = %i capacity is %1.3f and preflow is %1.3f.\n", k,
	    capacity(cap, node, k), preflow(pre, node, k));
    */
