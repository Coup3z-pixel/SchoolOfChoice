...
struct partial_alloc MCC_alloc(struct process_scp* myscp) {
  ...
  if (OLD_allocation_is_efficient(&answer, &reduced) &&
      !NEW_allocation_is_efficient(&answer, &reduced)) {
    fprintf(stderr, "The allocation is OLD_efficient but not NEW_efficient.\n");
  }

  if (!OLD_allocation_is_efficient(&answer, &reduced) &&
      NEW_allocation_is_efficient(&answer, &reduced)) {
    fprintf(stderr, "The allocation is NEW_efficient but not OLD_efficient.\n");
  }
  
  /*
  if (NEW_allocation_is_efficient(&answer, &reduced)) {
