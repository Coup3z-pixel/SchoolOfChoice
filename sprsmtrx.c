...
int int_entry(struct int_sparse_matrix* mymat, int row_no, int col_index) {
  ...
  while (probe <= mymat->nos_active_cols[row_no-1] &&
	 mymat->index_of_active_cols[row_no-1][probe-1] != col_index) {
    probe++;
  }
...
struct int_sparse_matrix zero_int_sp_mat_from_dbl_sp_mat(struct dbl_sparse_matrix* given) {
  ...
  struct int_sparse_matrix template;

  template.no_rows = given->no_rows;
  template.no_cols = given->no_cols;

  no_rows = template.no_rows;

  template.nos_active_cols = malloc(no_rows * sizeof(int));
  for (i = 1; i <= no_rows; i++) {
    template.nos_active_cols[i-1] = given->nos_active_cols[i-1];
  }

  template.index_of_active_cols = malloc(no_rows * sizeof(int*));
  for (i = 1; i <= no_rows; i++) {
    template.index_of_active_cols[i-1] = malloc(template.nos_active_cols[i-1] * sizeof(int));
    for (j = 1; j <= template.nos_active_cols[i-1]; j++) {
      template.index_of_active_cols[i-1][j-1] = given->index_of_active_cols[i-1][j-1];
    }
  }

  template.entries = malloc(no_rows * sizeof(int*));
  for (i = 1; i <= no_rows; i++) {
    template.entries[i-1] = malloc(template.nos_active_cols[i-1] * sizeof(int));
    for (j = 1; j <= template.nos_active_cols[i-1]; j++) {
      template.entries[i-1][j-1] = 0;
    }
  }
