  ...
  */
  ...
  if (argc == 8) {
	...
	|| test_std_dev < 0.000001) {
      printf("The fourth, fifth, and sixth command line arguments must be  floats > 0.0.\n");
      exit(0);
    }
    if (no_priority_grades <= 1) {
      printf("The final command line argument must be an integer > 1.\n");
      exit(0);
    }
  }
  ...
  if (argc == 5) {
	...
	no_priority_grades <= 0) {
      printf("All command line arguments (after make_ex) must be integers > 0.\n");
      exit(0);
    }
    school_valence_std_dev = 1.0;
    idiosyncratic_std_dev = 1.0;
