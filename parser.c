...
void check_There_second(FILE *fp, char next) {
  if (next != 'T' || getc(fp) != 'h' || getc(fp) != 'e' || getc(fp) != 'r' || getc(fp) != 'e') {
    printf("ERROR: formatting for number of schools is incorrect.\n");
    exit(0);
  }
}

void check_are_fifth(FILE *fp, char next) {
  if (next != 'a' || getc(fp) != 'r' || getc(fp) != 'e') {
    printf("ERROR: formatting for the number of schools is incorrect.\n");
    exit(0);
  }
}

void check_schools_fifth(FILE *fp, char next) {
  if (next != 's' || getc(fp) != 'c' || getc(fp) != 'h' || getc(fp) != 'o' || getc(fp) != 'o' ||
                     getc(fp) != 'l' || getc(fp) != 's') {
    printf("ERROR: formatting for the number of schools is incorrect.\n");
    exit(0);
...
struct square_matrix related_matrix_from_file() {
  ...
  char next = get_next(fp);
  check_There_second(fp,next);
  next = get_next(fp);
  check_are_fifth(fp,next);
  next = get_next(fp);
  related.dimension = get_number(fp,next);
  next = get_next(fp);
  ...
  for (j = 1; j <= related.dimension; j++) {
    related.entries[j-1] = malloc(related.dimension * sizeof(int));
    for (k = 1; k <= related.dimension; k++) {
      next = get_next(fp);
      related.entries[j-1][k-1] = get_number(fp,next);
    }
  }
  ...
  for (j = 1; j <= related.dimension; j++) {
    for (k = 1; k <= related.dimension; k++) {
      if (!(related.entries[j-1][k-1] == 0 || related.entries[j-1][k-1] == 1) ||
	  related.entries[j-1][k-1] != related.entries[k-1][j-1]) {
	printf("ERROR: The matrix in related.mat must be a symmetric 0,1-matrix.\n");
	exit(0);
      }
    }
  }
