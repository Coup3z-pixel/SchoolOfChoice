...
int* popular_schools(struct double_cee* my_cee, double time_remaining) {
  ...
  for (j = 1; j <= my_cee->no_schools; j++) {
    ...
    if ((double)demand * time_remaining <= my_cee->quotas[j-1]) {
      pop[j-1] = 0;
    }
    else {
      pop[j-1] = 1;
...
void remove_unpopular_schools(struct square_matrix* raw_related, int* popular) {
  ...
  int nsc = raw_related->dimension;
  
  for (j = 1; j <= nsc; j++) {
    for (k = 1; k <= nsc; k++) {
      if (popular[j-1] == 0 || popular[k-1] == 0) {
	raw_related->entries[j-1][k-1] = 0;
      }
    }
  }
}
...
struct square_matrix related_matrix(struct double_cee* my_cee, double time_remaining) {
  ...
  int* popular = popular_schools(my_cee, time_remaining);

  struct square_matrix relatedness = raw_related_matrix(my_cee);

  remove_unpopular_schools(&relatedness,popular);
...
struct square_matrix related_matrix(struct double_cee* my_cee) {
  ...
  int* popular = popular_schools(my_cee, 1.0);
